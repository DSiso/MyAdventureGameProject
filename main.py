...
def intro():
    print_pause("You find yourself in a small village during Stormy weather "
                "late in the evening.")
...
def naughty_boys(item, play_again_list):
    if item == "candies":
        print_pause("Candies made the naughty_boys very happy.")
        print_pause("They let you continue your way.")
    else:
        if item == "guns":
            print_pause("Your guns scared the naughty_boys and "
                        "made them cry.")
        elif item == "holy water":
            print_pause("Holy water isn't something naughty_boys expect "
                        "to get at midnight!. It only made them cry.")
...
def ghosts(item, play_again_list):
    if item == "guns":
        print_pause("With your guns you shot directly to ghosts' heads.")
        print_pause("All zombies died! You may continue your way.")
    elif item == "candies" or item == "holy water":
        print_pause("This wasn't effective against ghosts!")
        print_pause("You lost! You've also become a ghost and "
...
def vampires(item, play_again_list):
    if item == "holy water":
        ...
        print_pause("This wasn't effective against vampires!")
...
def action_1(enemy, item, items_trunk, play_again_list, enemies):
    ...
    if enemy == "naughty_boys":
        naughty_boys(item, play_again_list)
    elif enemy == "ghosts":
        ghosts(item, play_again_list)
    elif enemy == "vampires":
...
def play_game():
    ...
    enemies = ["naughty_boys", "ghosts", "vampires"]
    actions = ['1', '2']
